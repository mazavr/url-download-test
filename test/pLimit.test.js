...
test('concurrency: 1', t => {
    ...
    return Promise.all([100, 200, 300, 400].map(ms => limit(() => delay(ms, {value: ms})))).then(res => {
        let ended = end();
        t.true(inRange(ended, getEnd(1000)));
        t.deepEqual(res, [100, 200, 300, 400])
    })
});
